...
export interface Exercise {
  ...
  customId?: string
  /*
   * Custom exercises carry an absolute image URL (Supabase Storage) rather than a filename
   * inside the CDN's category folder, so `img` cannot express them — this is why a
   * user-uploaded photo previously never appeared anywhere.
   */
  imageUrl?: string
...
export interface CustomExerciseRow {
  ...
  form: string | null
  /*
   * Everything below arrived with shared custom exercises (docs/SUPABASE-MIGRATION.md) and is
   * optional on purpose: a database that has not had the migration applied returns rows without
   * these keys, and the apps carry on with the legacy shape rather than breaking.
   */
  /** public URL of a user-uploaded photo, in the `exercise-images` storage bucket */
