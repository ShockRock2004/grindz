...
/*
 * Length floors for the generated coaching text.
 *
 * Measured from the 34 built-in form cues and their tips rather than picked by feel: cues run
 * 30-58 characters (median 36) and tips 45-68. Left to itself the model returns things like
 * "Keep your back straight" — grammatical, useless, and visibly thinner than the catalogue
 * entry sitting next to it on the same screen. These are the shortest real examples, so
 * anything under them is shorter than anything a human wrote.
 */
const MIN_CUE = 30
const MAX_CUE = 90
const MIN_TIP = 45
...
export interface Verdict {
  ...
  formCue: string
  /**
   * Exactly three short "do this better" pointers. These are what make a custom exercise
   * read like a built-in one — the catalogue entries all carry coaching detail, and an
   * exercise the user typed in themselves looked bare next to them without it.
   */
  tips: string[]
...
- "tips": EXACTLY 3 coaching pointers that would improve the lift. Each MUST be between 45 and 120 characters — a full, specific instruction, not a fragment. Cover three different things: setup, execution, and a common mistake to avoid. Each must mention something concrete (a joint, an angle, a tempo, a cue). GOOD: "Set the bench near 30 degrees — steeper shifts the load onto the front delts." TOO SHORT: "Keep your core tight.", "Do not bounce.", "Control the weight."
...
function shortfall(v: Verdict): string | null {
  ...
  if (thin.length) {
    problems.push(
      `these tips are too short (minimum ${MIN_TIP} characters): ${thin.map((t) => `"${t}" (${t.length})`).join(', ')}`,
    )
  }
...
export async function validateExercise(draft: ExerciseDraft): Promise<Verdict> {
   ...
   *
   * Stating the minimums in the system prompt gets most of the way there, but the model still
   * occasionally answers "Keep your back straight." Quoting its own too-short output back with
   * the measured character count is far more effective than repeating the rule, and a slightly
   * higher temperature stops it returning the identical string again.
   *
   * Exactly one retry, and the original is kept if the second attempt is no better: this is an
   * advisory feature, so a thin cue must never block someone from saving their exercise.
   */
  ...
  // keep whichever is actually usable; prefer the retry only when it fixed something
