  ...
  return CATALOG.map((c) => {
    const extra: Exercise[] = custom
      ...
      .map((x) => ({
        ...
        customId: x.id,
        mode: x.mode === 'timed' || x.mode === 'distance' ? x.mode : undefined,
        imageUri: x.image_url ?? undefined,
...
/**
 * Logging mode for an exercise. Custom exercises carry their own mode, chosen when
 * they were added, so a custom plank logs in seconds exactly like a built-in one.
 */
export function exerciseMode(name: string, custom: CustomExerciseRow[] = []): 'reps' | 'timed' | 'distance' {
  for (const c of CATALOG) {
    const e = c.exercises.find((x) => x.name === name)
    if (e) return e.mode ?? 'reps'
  }
  const cx = custom.find((x) => x.name === name)
  const m = cx?.mode
