...
It is **deliberately incomplete** and grows over time. `apps/web/src/data/femaleAssets.ts`
(regenerated by `apps/web/scripts/gen-female-assets.mjs` — run it after adding or
removing a file here, then commit the result) is the honest record of what actually
exists; `cdnExercise`/`cdnHero` in `data/assetCdn.ts` check it and fall back to the male
photo for anything not shot yet, so no call site ever has to know the set is partial.

Adding a female photo is the same "commit, don't upload" flow as the base library:

```bash
cp new-photo.png cdn/public/female/images/legs/barbell-squats.png
node apps/web/scripts/gen-female-assets.mjs   # regenerate the manifest both apps read
git add cdn/public/female/images/legs/barbell-squats.png apps/web/src/data/femaleAssets.ts apps/mobile/src/data/femaleAssets.ts
git commit && git push
```
